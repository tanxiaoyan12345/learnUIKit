...
- (void)endEdit{
    ...
    //间隔时设置时间的两种情况
    [self setInterval:self.isRuning];
}

- (void)setInterval:(BOOL)isRuning {
    if(isRuning == NO) {
        if (self.newSleeptime>self.sleeptime) {
        ...
        }else if (self.newSleeptime<self.sleeptime) {
            self.continueFlag = YES;
            [self stopEvent:self.continueFlag];
            self.continueFlag = NO;
            [self startEvent];
        }
    }
...
//自定义了一个myOperation,为了熟悉自定义operation的使用 把前两个任务放在了myOperation中
- (void) blockOperation {
    NSLog(@"Task%ld Begin!",self.count);
    Task *task = [[Task alloc] init];
    _queue = [[NSOperationQueue alloc]init];
...
- (void)stopEvent:(BOOL)continueFlag {
